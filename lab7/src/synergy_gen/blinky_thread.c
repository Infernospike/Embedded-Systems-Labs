...
TX_MUTEX g_mutex0;
TX_QUEUE g_queue0;
static uint8_t queue_memory_g_queue0[20];
...
void blinky_thread_create(void)
{
    ...
    UINT err_g_mutex0;
    err_g_mutex0 = tx_mutex_create (&g_mutex0, (CHAR*) "New Mutex", TX_NO_INHERIT);
    if (TX_SUCCESS != err_g_mutex0)
    {
        tx_startup_err_callback (&g_mutex0, 0);
    }
    UINT err_g_queue0;
    err_g_queue0 = tx_queue_create (&g_queue0, (CHAR*) "New Queue", 1, &queue_memory_g_queue0,
                                    sizeof(queue_memory_g_queue0));
    if (TX_SUCCESS != err_g_queue0)
    {
        tx_startup_err_callback (&g_queue0, 0);
    }
